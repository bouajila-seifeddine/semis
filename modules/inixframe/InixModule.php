...
class Inix2Module extends Module {
	...
	private $feedback_options = array(
		'most'=> array( 'features','usability','design','maintainability','support','flexibility','efficiency','security','extensibility'),
		'company'=> array( 'our products', 'the quality of our distribution', 'the price of your distribution', 'our customer service' ),
		'better' => array('more features','better usability','modern design','maintainability','faster support','more flexibility','efficiency','more security','extensibility','easy navigation','more features'),
	...
	/**
	 * @since 1.5.0.1
	 * @var string Module local path (eg. '/home/prestashop/modules/inixframe/')
	 */
	...
	public function __construct()
	{
		...
		if(!$this->context->controller instanceof AdminModulesController) return;
	...
	public function processFilter()
	{
		...
		if (isset($this->object_table)) {
			foreach ($_POST as $key => $value) { // key contains object_table
		...
		foreach ($filters as $key => $value)
		{
			...
			if ($value != null && !strncmp($key, $this->cookie_prefix.$this->object_table.'Filter_', 7 + Tools::strlen($this->cookie_prefix.$this->object_table)))
			{
	...
	public function ajaxProcesshideWellcome(){
		$this->configPut('FRAME_W_'.strtoupper($this->name),1);
		die('1');
	}
	public function ajaxProcesshideConscent(){
		$this->configPut('IWFRAME_CONSCENT_'.strtoupper($this->name),1);
		die('1');
	}
	...
	public function ajaxProcessRegister(){
		...
		} else if (!Validate::isEmail(Tools::getValue('update_service_email'))){
		...
		else if (!Validate::isPasswd(Tools::getValue('update_service_password')))
			$this->errors[] = sprintf($this->l('Password should be at least %d charactes long'),Validate::PASSWORD_LENGTH);


		if(!count($this->errors)){

			$client = new InixUpdateClient($this->configGet('IWFRAME_CLIENT_TOKEN'));
			$response = $client->register(Tools::getValue('update_service_email'),Tools::getValue('update_service_password'));
			...
			elseif($client->getStatus() =='error'){
				...
				if(!isset($response['client_token'])){
					...
					$this->configPut( 'IWFRAME_CLIENT_TOKEN',$response['client_token']);
	...
	public function processDelete()
	{
		if (Validate::isLoadedObject($object = $this->loadObject()))
		{
			...
			else
			{
				...
				self::addLog(sprintf($this->l('%s deletion', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int)$this->object->id, true, (int)$this->context->employee->id);
	...
	public function processAdd()
	{
		...
		if (count($this->errors) <= 0)
		{
			...
			{
				self::addLog(sprintf($this->l('%s addition', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int)$this->object->id, true, (int)$this->context->employee->id);
				...
				if (Validate::isLoadedObject($object))
				{
					...
					if (!$result)
					{
						$this->errors[] = Tools::displayError('An error occurred while updating an object.').
							' <b>'.$this->object_table.'</b> ('.Db::getInstance()->getMsgError().')';
					}
					...
					self::addLog(sprintf($this->l('%s edition', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int)$object->id, true, (int)$this->context->employee->id);
	...
	public function processResetFilters()
	{
		...
		foreach ($filters as $cookie_key => $filter)
			if (strncmp($cookie_key, $this->cookie_prefix.$this->object_table.'Filter_', 7 + Tools::strlen($this->cookie_prefix.$this->object_table)) == 0)
			{
	...
	public function assignUpdate() {
		$module_remote_data = @json_decode( $this->configGet( 'IWFRAME_REMOTE_DATA' ), true );
	...
	public function cleanUpdateData() {
		$this->configDelete( 'IWFRAME_LAST_UPDATE_CHECK' );
		$this->configDelete( 'IWFRAME_OWN_MODULES' );
		$this->configDelete( 'IWFRAME_OWN_MODULES_FORMATED' );
		$this->configDelete( 'IWFRAME_REMOTE_DATA' );
	}

	public function shouldCheckForUpdate( ){
		if($this->configGet( 'IWFRAME_LAST_UPDATE_CHECK' ) == false)
			return true;

		$last_update_check = $this->configGet( 'IWFRAME_LAST_UPDATE_CHECK' );
	...
	public function installMailTpl( $tpl_name ) {
		...
		foreach ( $langs as $l ) {
			...
			$mails &= @copy( $this->getLocalPath() . '/mails/' . $tpl_name . '.html', $this->getLocalPath() . '/mails/' . $l['iso_code'] . '/' . $tpl_name . '.html' );
			if ( file_exists( $this->getLocalPath() . '/mails/' . $tpl_name . '.txt' ) ) {
				$mails &= @copy( $this->getLocalPath() . '/mails/' . $tpl_name . '.txt', $this->getLocalPath() . '/mails/' . $l['iso_code'] . '/' . $tpl_name . '.txt' );
			} else {
				$mails &= @copy( $this->getLocalPath() . '/mails/' . $tpl_name . '.html', $this->getLocalPath() . '/mails/' . $l['iso_code'] . '/' . $tpl_name . '.txt' );
			}
		}
	...
	protected function updateCheck(&$errors = array()) {
		if(class_exists('InixUpdateClient')) {
			if ( ! isset( $this->client ) ) {
				$this->client = new InixUpdateClient($this->configGet('IWFRAME_CLIENT_TOKEN'));
			}
			$own_modules = $this->getOurModules();
			$response    = $this->client->checkUpdate( $own_modules );

			if($response === false){
				$this->configPut( 'IWFRAME_LAST_UPDATE_CHECK', time() );
				$errors[] = $this->l('Unable to connect to the update service');
			} elseif ( $this->client->getStatus() == 'ok' ) {
				$this->configPut( 'IWFRAME_LAST_UPDATE_CHECK', time() );
				$this->configPut( 'IWFRAME_REMOTE_DATA', json_encode( $response ) );
	...
	public function getOurModules() {
		...
		$this->configPut( 'IWFRAME_OWN_MODULES', json_encode( $ownmodules ) );
		$this->configPut( 'IWFRAME_OWN_MODULES_FORMATED', json_encode( $pretty_modules ) );
				...
				if ((!Shop::isFeatureActive() && isset($values['required']) && $values['required'])
					...
					if (isset($values['type']) && ( $values['type'] == 'textLang' OR $values['type'] == 'textareaLang'))
					{
						...
						if($value_default_lang == false)
							$this->errors[] = sprintf(Tools::displayError('field %s is required at least in default language.'), $values['title']);
					...
					if (isset($values['type']) && $values['type'] == 'textLang' OR $values['type'] =='textareaLang')
					{
						...
						foreach ($languages as $language){
							...
							if (( $value_default_lang OR $value )  && isset($values['validation']))
								if (!Validate::$values['validation'](Tools::getValue($field.'_'.$language['id_lang'],Tools::getValue($field.'_'.$id_lang_default))))
									$this->errors[] = sprintf(Tools::displayError('field %s is invalid.'), $values['title']);
						}
				...
				foreach ($fields as $key => $options)
				{
					...
					else
					{
						...
						if ($this->validateField($val, $options))
						{
						    ...
						    if (isset($options['is_array']) AND $options['is_array'])
								Configuration::updateValue($key, json_encode($val),isset($options['html'])? $options['html'] : false);
							else if (Validate::isCleanHtml($val))
								Configuration::updateValue($key, $val,isset($options['html'])? $options['html'] : false);
							else
	...
	protected  function processAddfeedback(){
		...
		if(!count($this->errors)){
			foreach($this->feedback_labels as $name => $label) {
				if($name == 'yes' OR $name =='no') continue;
				if($name == 'other_yes' AND Tools::getValue('other') ==0) continue;
			...
			if(Inix2Mail::FrameSend('feedback','[Feedback] '.$this->author , $template_vars,Tools::getValue('email'),Tools::getValue('name'),null)){
				$this->configPut('FRAME_GOT_FEEDBACK',1);
	...
	protected  function processAddBugreport(){
		...
		if(!count($this->errors)){
			...
			if(Inix2Mail::FrameSend('bugreport','[BUG REPORT] '.$this->displayName.' v'.$this->version  , $template_vars,Tools::getValue('email'), Configuration::get('PS_SHOP_NAME'), $file_attachment,$this)){
				$this->context->smarty->assign('conf',true);
			} else {
	...
	public function displayContent()
	{
		...
		$dir = $this->getLocalPath().'views'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$this->override_folder;
		if( !file_exists($dir.'page_header_toolbar.tpl')){
			$dir = $this->getFrameLocalPath().DIRECTORY_SEPARATOR.'template'.DIRECTORY_SEPARATOR;
		}
		$page_header_toolbar = ($dir.'page_header_toolbar.tpl');
	...
	public function init()
	{
		...
		if (!$this->object_identifier) $this->object_identifier = 'id_'.$this->object_table;
		if (!$this->_defaultOrderBy) $this->_defaultOrderBy = $this->object_identifier;
		...
		$this->context->smarty->assign(array(
			...
			'got_feedback' => $this->configGet('FRAME_GOT_FEEDBACK'),
			'show_wellcome' => $this->show_wellcome AND !$this->configGet('FRAME_W_'.strtoupper($this->name)),
			...
			'show_register'=> ($this->name != 'inixframe' AND $this->configGet( 'IWFRAME_CLIENT_TOKEN' ) == false AND $this->configGet( 'IWFRAME_REG_'.strtoupper($this->name)  )== false ),
			'show_conscent' => ($this->name != 'inixframe' AND !Module::isInstalled('inixframe') AND $this->configGet( 'IWFRAME_CLIENT_TOKEN' ) == false AND $this->configGet( 'IWFRAME_CONSCENT_'.strtoupper($this->name)  )== false )
		));
	...
	public function getList($id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false)
	{
		...
		if ($this->multishop_context && Shop::isTableAssociated($this->object_table) && !empty($this->className))
		{
			...
			{
				$test_join = !preg_match('#`?'.preg_quote(_DB_PREFIX_.$this->object_table.'_shop').'`? *sa#', $this->_join);
		...
		if (!($this->_list = Db::getInstance()->executeS($this->_listsql)))
			...
			$this->_listTotal = Db::getInstance()->getValue('SELECT FOUND_ROWS() AS `'._DB_PREFIX_.$this->object_table.'`');
	}
				...
				foreach ($fieldset['form']['input'] as $input)
					if (!isset($this->fields_value[$input['name']]))
						if (isset($input['type']) && $input['type'] == 'shop')
						{
							if ($obj->id)
							{
								$result = Shop::getShopById((int)$obj->id, $this->object_identifier, $this->object_table);
	...
	protected function updateAssoShop($id_object)
	{
		...
		Db::getInstance()->delete($this->object_table.'_shop', '`'.$this->object_identifier.'` = '.(int)$id_object.($exclude_ids ? ' AND id_shop NOT IN ('.implode(', ', $exclude_ids).')' : ''));
		...
		if (isset($field['validation']) )
		{
			...
			if ((!isset($field['empty']) || !$field['empty'] || (isset($field['empty']) && $field['empty'] && $value)) && $valid_method_exists)
			{
				if(isset($field['is_array']) AND $field['is_array']){
					foreach($value as $v){
						if(!Validate::$field['validation']($v)){
							$this->errors[] = Tools::displayError($field['title'].' : Has incorrect values');
							return false;
						}
					}
				}
				else if (!Validate::$field['validation']($value))
				{
	...
	protected function processBulkAffectZone()
	{
		...
		{
			$object = new $this->className();
			$result = $object->affectZoneToSelection(Tools::getValue($this->object_table.'Box'), Tools::getValue('zone_to_affect'));
	...
	public function install(){
		...
		foreach($this->install_tabs as $t){
			$tabs &= $this->installModuleTab($t['class'],$t['name'],$t['parent'],(isset($t['active']) ? $t['active'] : true));
		}
	...
	public function createTemplate($tpl_name)
	{
		...
		if (file_exists($this->getLocalPath().'views'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$this->override_folder.$tpl_name))
			return $this->context->smarty->createTemplate($this->getLocalPath().'views'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$this->override_folder.$tpl_name, $this->context->smarty);

		return $this->context->smarty->createTemplate($this->getFrameLocalPath().DIRECTORY_SEPARATOR.'template'.DIRECTORY_SEPARATOR.$tpl_name, $this->context->smarty);
	}
	...
	public static function addLog($message, $severity = 1, $error_code = null, $object_type = null, $object_id = null, $allow_duplicate = false, $id_employee = null){
		...
		if(class_exists('PrestaShopLogger'))
			PrestaShopLogger::addLog($message, $severity = 1, $error_code = null, $object_type = null, $object_id = null, $allow_duplicate = false, $id_employee = null);
		else
			Logger::addLog($message, $severity = 1, $error_code = null, $object_type = null, $object_id = null, $allow_duplicate = false, $id_employee = null);
	}
	...
	public function configLoad() {
		...
		$config = unserialize(Tools::file_get_contents($this->getFrameLocalPath().'config/config.data'));
	...
	public function configWrite() {
		file_put_contents($this->getFrameLocalPath().'config/config.data',serialize($this->configGetAll()));
	}
}