 ...
 * @since 1.5.0
 */
class Inix2HelperList extends Inix2Helper
{
		...
		foreach ($this->_list as $index => $tr)
		{
			...
			foreach ($this->actions as $action)
			{
				...
				if (!array_key_exists($action, $this->list_skip_actions) || !in_array($id, $this->list_skip_actions[$action]))
				{
					...
					if (method_exists($this->module, $method_name))
						$this->_list[$index][$action] = $this->module->$method_name($this->token, $id, $name,$tr);
					elseif (method_exists($this->context->controller, $method_name))
						$this->_list[$index][$action] = $this->context->controller->$method_name($this->token, $id, $name,$tr);
					elseif (method_exists($this, $method_name))
						$this->_list[$index][$action] = $this->$method_name($this->token, $id, $name,$tr);
				}
			}
			...
			foreach ($this->fields_list as $key => $params)
			{
				...
				if (isset($params['active']))
				{
					...
					if (method_exists($this->module, 'displayEnableLink'))
				...
				elseif (isset($params['position']))
				{
					$this->_list[$index][$key] = array(
						...
						'position_url_down' => $this->currentIndex.
							...
							'&way=1&fposition='.((int)$tr['position'] + 1).'&token='.$this->token,
						'position_url_up' => $this->currentIndex.
							(isset($key_to_get) ? '&'.$key_to_get.'='.(int)$position_group_identifier : '').
							'&'.$this->position_identifier.'='.$id.
							'&way=0&fposition='.((int)$tr['position'] - 1).'&token='.$this->token
					);
				}
				...
				elseif (isset($tr[$key]))
				{
					...
					{
						$callback_obj = (isset($params['callback_object'])) ? $params['callback_object'] : $this->module;
	...
	public function displayListHeader()
	{
		...
		if (in_array((int)Tools::getValue($this->table.'_pagination'), $this->_pagination))
			$pagination = (int)Tools::getValue($this->table.'_pagination');
		elseif (isset($this->context->cookie->{$this->module->name.$this->table.'_pagination'}) && $this->context->cookie->{$this->table.'_pagination'})
			$pagination = $this->context->cookie->{$this->module->name.$this->table.'_pagination'};
		...
		$selected_pagination = Tools::getValue(
			$this->table.'_pagination',
			isset($this->context->cookie->{$this->module->name.$this->table.'_pagination'}) ? $this->context->cookie->{$this->module->name.$this->table.'_pagination'} : $this->_default_pagination
		);
		...
		$this->header_tpl->assign(array_merge($this->tpl_vars, array(
			...
			'show_filters' => true,//(count($this->_list) <= 1 && !$has_value) ? false : true,
