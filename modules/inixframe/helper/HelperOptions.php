			...
			foreach ($category_data['fields'] as $key => $field)
			{
				...
				if($field['type']=='products'){
					if($products){
						...
						foreach($temp_products as $product_id){
							...
							$res = DB::getInstance()->getRow('SELECT p.id_product,p.reference, pl.name FROM `'._DB_PREFIX_.'product` p
				...
				if($field['type']=='categories'){
					if ($categories)
					{
						...
						$tree = new InixHelper2TreeCategories($field['tree']['id'], isset($field['tree']['title']) ? $field['tree']['title'] : null);
				...
				if (substr($field['type'], -4) == 'Lang')
				{
					foreach ($languages as $language)
					{
						...
						$field['value'][$language['id_lang']] = $this->getOptionValue($key.'_'.strtoupper($language['id_lang']), $field);
					}
				}
				if($field['type'] == 'date'){
