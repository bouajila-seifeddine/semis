...
class InixHelper2TreeShops extends Inix2Tree
{
	...
	public function render($data = null, $use_default_actions = true, $use_selected_shop = true)
	{
		...
		if ($use_default_actions) {
			$treeToolbarLink = new Inix2TreeToolbarLink(
				'Collapse All',
				'#',
				'$(\'#' . $this->getId() . '\').tree(\'collapseAll\'); return false;',
				'icon-collapse-alt');
			$treeToolbarLink->setModule($this->getModule());
			$treeToolbarLink1 = new Inix2TreeToolbarLink(
				'Expand All',
				'#',
				'$(\'#' . $this->getId() . '\').tree(\'expandAll\'); return false;',
				'icon-expand-alt');
			$treeToolbarLink1->setModule($this->getModule());
			$treeToolbarLink2 = new Inix2TreeToolbarLink(
				'Check All',
				'#',
				'checkAllAssociatedShops($(\'#' . $this->getId() . '\')); return false;',
				'icon-check-sign');
			$treeToolbarLink2->setModule($this->getModule());
			$treeToolbarLink3 = new Inix2TreeToolbarLink(
				'Uncheck All',
				'#',
				'uncheckAllAssociatedShops($(\'#' . $this->getId() . '\')); return false;',
				'icon-check-empty');
			$treeToolbarLink3->setModule($this->getModule());
