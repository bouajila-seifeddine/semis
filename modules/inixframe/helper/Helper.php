...
class Inix2Helper extends  Helper
{
	public $currentIndex;
	public $table;
	public $identifier;
	...
	{
		$this->context->smarty->addTemplateDir($this->module->getFrameLocalPath(),3);


		if($this->module){

			if (file_exists( $this->module->getLocalPath().'/views/templates/'.$this->override_folder.$this->base_folder.$tpl_name))
				$override_tpl_path = $this->module->getLocalPath().'/views/templates/'.$this->override_folder.$this->base_folder.$tpl_name;
			else if (file_exists($this->module->getFrameLocalPath().'template/'.$this->base_folder.$tpl_name))
				$override_tpl_path =$this->module->getFrameLocalPath().'template/'.$this->base_folder.$tpl_name;
...
//			$html .= 'buildTreeView('.$use_shop_context.');';
		...
		if ($root['id_category'] != $top_category->id || (Tools::isSubmit('ajax') && Tools::getValue('action') == 'getCategoriesFromRootCategory'))
			$root_input = '<p class="checkbox"><i class="icon-folder-open"></i><label>
