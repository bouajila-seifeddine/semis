...
class inixframe extends Inix2Module {
	...
	public function getContent() {
		...
		if(!Tools::isSubmit('ajax') && ( $this->shouldCheckForUpdate() OR  Tools::isSubmit('refresh')) ){
			...
			if(is_array($banners) AND isset($banners['banners']))
				$this->configPut('IWFRAME_BANNERS',$banners['banners']);

			if($this->configGet( 'IWFRAME_REMOTE_DATA' ) === false)
		...
		$own_modules_formated = json_decode($this->configGet( 'IWFRAME_OWN_MODULES_FORMATED' ),true);


		$saved_banners = $this->configGet('IWFRAME_BANNERS',array());
	...
	public function ajaxProcessUpgrade(){
		...
		if(!count($this->errors)) {
			...
			if ( $module->installed ) {
				$sql = new DbQuery();
				$module->database_version = DB::getInstance()->getValue($sql->select('version')->from('module')->where('name = \''.pSQL($module->name).'\''));
				$v = '';
