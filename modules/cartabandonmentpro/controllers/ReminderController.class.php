...
class ReminderController
{
    ...
    public static function getAbandonedCart($wichReminder, $id_shop = 1)
    {
        ...
        $query    =  'SELECT `ca`.*, `cu`.`firstname`, `cu`.`lastname`, `cu`.`id_customer`, `cu`.`email`, MAX(ord.id_order) as id_order
        ...
        if ($wichReminder > 1) {
            ...
            $query .= 'ca.id_cart IN (
                ...
                WHERE wich_remind = ' . (int)$remind . ')
                       AND ';
        }
        $query .= ' ca.id_cart NOT IN (
                        SELECT cr.id_cart
                        FROM '._DB_PREFIX_.'cartabandonment_remind cr
                        WHERE wich_remind = ' . (int)$wichReminder. '))
        ...
        $query .= ' HAVING id_order IS NULL AND ca.date_upd <= "' . pSql($reminder) . '"';
