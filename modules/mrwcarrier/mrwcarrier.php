 ...
 *
 * @author MRW Iberia - Dpto. Desarrollo
 * @copyright (c)2015 - MRW IBERIA
 * @version 2.1.19 - 06/04/2016
 *
 */
...
class MrwCarrier extends Module
{
    ...
    public function install()
    {
        if (!parent::install()
            ...
            $carrierConfig = array('name' => 'MRW - ' . $this->name,
                'id_tax_rules_group' => 0,
                'active' => true,
                'deleted' => 0,
                'shipping_handling' => false,
                'range_behavior' => 0,
                'delay' => 'Entrega en 24 horas',
                'id_zone' => 1,
                'is_module' => false,
                'shipping_external' => false,
                'external_module_name' => $this->name,
                'need_range' => true,
                'url' => $this->urlTracking,
                'grade' => '3',
            );

            $carrierStandardConfig = array('name' => 'MRW - Entrega en Franquicia',
                'id_tax_rules_group' => 0,
                'active' => true,
                'deleted' => 0,
                'shipping_handling' => false,
                'range_behavior' => 0,
                'delay' => 'Entrega en Franquicia',
                'id_zone' => 1,
                'is_module' => false,
                'shipping_external' => false,
                'external_module_name' => $this->name || '_EF',
                'need_range' => true,
                'url' => $this->urlTracking,
                'grade' => '4',
            );

            // Creamos el nuevo carrier y obtenemos su ID
            $lastId = $this->installExternalCarrier($carrierConfig);
            $lastStandardId = $this->installExternalCarrier($carrierStandardConfig);
            if ($lastId === false || $lastStandardId === false) {
                return false;
            }
            // Ha fallado la creacion del carrier
            else {
                $this->writeToFileParam('Carrier creado: (' . $lastId . ')');
                ...
                if ($rowCarrier) {
                    ...
                    if ($rowCarrier && isset($rowCarrier)) {
                        ...
                        //$existsIdShopQuery = 'SELECT COUNT(*) FROM INFORMATION_SCHEMA\.COLUMNS WHERE TABLE_NAME = ' . _DB_PREFIX_ . $this->name . '_mrw AND COLUMN_NAME = id_shop';
                ...
                $sql_mrw_test = 'CREATE TABLE IF NOT EXISTS ' . _DB_PREFIX_ . 'mrwtest ( check_field tinyint(1) NOT NULL DEFAULT 0) CHARACTER SET utf8 COLLATE utf8_general_ci';
                $sql_mrw_test = Db::getInstance()->Execute($sql_mrw_test);
                if (!$sql_mrw_test) {
                    $this->writeToFileParam('ERROR al crear la tabla ' . _DB_PREFIX_ . 'mrwtest)');
                    return false;
                }

                //Crear campo check
                $sql_mrw_test = 'INSERT INTO ' . _DB_PREFIX_ . 'mrwtest (check_field) VALUES (0)';
                $sql_mrw_test = Db::getInstance()->Execute($sql_mrw_test);

                //Poner valor a 0 por defecto para que puedan entrar
                $sql_mrw_test =  'UPDATE ' . _DB_PREFIX_ . 'mrwtest SET check_field = 0';
                $sql_mrw_test = Db::getInstance()->Execute($sql_mrw_test);
                if (!$sql_mrw_test) {
                    $this->writeToFileParam('ERROR el introducir el valor 0 en el campo check_field de la tabla' . _DB_PREFIX_ . 'mrwtest)');
                    return false;
                }
    ...
    /**
     * Crea todos los parametros relacionados con el nuevo transportista
     * @param type $config
     * @return boolean
     */
    public static function installExternalCarrier($config)
    {
        ...
        if ((strncmp(_PS_VERSION_, "1.5", 3) == 0) || (strncmp(_PS_VERSION_, "1.6", 3) == 0)) {
        ...
        if ($carrier->add()) {
            ...
            // En caso de PS 1.5.x/1.6.x insertamos en las tablas ps_carrier_shop y ps_carrier_tax_rules_group_shop
            if ((strncmp(_PS_VERSION_, "1.5", 3) == 0) || (strncmp(_PS_VERSION_, "1.6", 3) == 0)) {
                Db::getInstance()->autoExecute(_DB_PREFIX_ . 'carrier_tax_rules_group_shop', array('id_carrier' => (int) ($carrier->id), 'id_shop' => '1', 'id_tax_rules_group' => '0'), 'INSERT');
            }
    ...
    public function uninstall()
    {
        ...
        && Configuration::deleteByName($this->rename . 'ACTIVE_DEBUG');

        // Uninstall SQL subscribers
        $sql_drop_table_A = 'DROP TABLE ' . _DB_PREFIX_ . $this->name . '_subs';
        if (!Db::getInstance()->Execute($sql_drop_table_A)) {
            $success &= false;
        }

        // Uninstall SQL check mrw
        $sql_drop_table_B = 'DROP TABLE ' . _DB_PREFIX_ . 'mrwtest';
    ...
    public function getContent()
    {
        $output = '
                  ...
                  <div style="float:right"><img src="../modules/' . $this->name . '/logo.gif" alt="mrw"></div>
                  ';
        ...
        if (Tools::isSubmit('submitMRW')) {
            ...
            if (!$this->_postErrors) {
                ...
                if ($entorno == 'mrw_pre') {
                    ...
                    $url = 'http://sagec.mrw.es/MRWEnvio.asmx';
                } else {
                    $url = '';
                }
    ...
    private function displayFormSubscriber()
    {
            ...
            $output .= '
                <tr' . $alt . '>
                    ...
                    <td>
                        ...
                        <form action="index.php?tab=' . Tools::safeOutput(Tools::getValue('tab')) . '&configure=' . Tools::safeOutput(Tools::getValue('configure')) . '&token=' . Tools::safeOutput(Tools::getValue('token')) . '&tab_module=' . Tools::safeOutput(Tools::getValue('tab_module')) . '&module_name=' . Tools::safeOutput(Tools::getValue('module_name')) . '&id_tab=1&action=delete&id_subscriber=' . (int) ($c['id_subscriber']) . '" method="post" class="form" style="float: left;">
        ...
        $output .= '
                ...
                <div class="margin-form">
                    <input type="text" name="mrw_franquicia" value="' . $agency . '" />
                    <p class="clear">' . $this->l('Ejemplo:') . ' 0XXXX</p>
                </div>
                <label>' . $this->l('Abonado') . '</label>
                <div class="margin-form">
                    <input type="text" name="mrw_abonado" value="' . $subscriber . '" />
                    <p class="clear">' . $this->l('Ejemplo:') . ' XXXXXX</p>
                </div>
                <label>' . $this->l('Departamento') . '</label>
                <div class="margin-form">
                    <input type="text" name="mrw_departamento" value="' . $department . '" />
                ...
                <div class="margin-form">
                    <input type="text" name="mrw_usuario" value="' . $user . '" />
                    <p class="clear">' . $this->l('Ejemplo:') . ' SGCXXXXXXXXXXXX</p>
                </div>
                <label>' . $this->l('Contraseña') . '</label>
                <div class="margin-form">
                    <input type="text" name="mrw_clave" value="' . $password . '" />
    ...
    private function _postProcessSubscriber()
    {
        ...
        // Delete Script
        if (Tools::getValue('action') == 'delete' && Tools::getValue('id_subscriber')) {
            ...
            // Display Results
            if ($resultDelete) {
                $this->_output .= $this->displayConfirmation($this->l('Settings updated'));
            } else {
                $this->_output .= $this->displayErrors($this->l('Settings failed'));
            }
        }
    }
    ...
    public function executeCreatePDFTicket($id_order = '')
    {
        if ($id_order != '') {
            ...
            //APORVINO (JORGE MIRAT): controla caso donde no es posible obtener la información del pedido mrw (e.g. cuando la peticion al SAGEC es denegada)
    ...
    public function getProductweight($id_order)
    {
		...
		//TRiBi
		if (Configuration::get('PS_WEIGHT_UNIT') == "gr"){
			$weight = $weight/1000;
		}
		//Fin TRiBi
   ...
   /**
    ...
    public function getPedidoMRW($order_id)
    {
        ...
        //echo Db::getInstance()->getValue($sql_mrw_carrier);
    ...
    public function insertTicketCarrier($id_order, $params_mrw, $nro_envio_mrw, $nid_shop)
    {
        ...
        if ($rowOrder && isset($rowOrder)) {
            ...
            'INSERT INTO ' . _DB_PREFIX_ . $this->name . '_mrw
                    (
                            `id`, `id_shop`,`order_id`, `send_num_mrw`, `params_mrw`,
     ...
     * @return Array, listado de datos del Abonado
     */
    private function getDefaultSubscriber($id_shop)
    {
        $sql_count = '
          SELECT count(*) as contador
          FROM `' . _DB_PREFIX_ . $this->name . '_subs`
    ...
    public function getParamsMRW($id_order, $mode)
    {
        ...
        {
            $ALaAtencionDe = $row['company'];
            $nom_full = $row['firstname'] . ' ' . $row['lastname'];
        }
        else
        {
            $nom_full = $row['company'];
            $ALaAtencionDe = $row['firstname'] . ' ' . $row['lastname'];
        }
        ...
        if ($rowServ && isset($rowServ) && $rowServ['service'] != '') {
            ...
            if ($this->debug_activo == '1') {
                $this->writeToFileParam('getParamsMRW:-> El pedido tiene modificado el servicio (' . $service_type . ') o la entrega en sábado (' . $entrega_sabado . ') o la entrega en franquicia (' . $entrega_franquicia . ') o el retorno (' . $retorno_activado . ')');
            }
        } else {
            if ($mode) {
                // Automático
        ...
        if ($entrega_franquicia == 'N') {
            if ($msg_type == 'sms' and $this->phoneValid($phone, 'movil') == 1) {
                ...
                if ($this->phoneValid($phone, 'movil') == 1) {
                    $arr_notificaciones['NotificacionRequest'][0] = array('CanalNotificacion' => '2', 'TipoNotificacion' => '4', 'MailSMS' => $phone);
        ...
        $parametros = array(
            'request' => array(
                'DatosEntrega' => array(
                    ...
                    , 'Telefono' => $phone//Opcional - Muy recomendable
                    , 'Contacto' => $nom_full//Opcional - Muy recomendable
                    , 'ALaAtencionDe' => $ALaAtencionDe//$row['firstname'], //Opcional - Se puede omitir.
    ...
    public function hookBackOfficeFooter($params)
    {
        ...
        $query = 'SELECT `check_field` FROM ' . _DB_PREFIX_ . 'mrwtest';
        $rowCarrier = Db::getInstance()->getValue($query);

        if($rowCarrier == 0){

        $query2 = 'UPDATE  ' . _DB_PREFIX_ . 'mrwtest SET `check_field` = 1';
        $setCheck = Db::getInstance()->Execute($query2);
            ...
            if ($this->checkTime($minutes)) {
                ...
                try {
                    ...
                    $time = time();
                    Configuration::updateValue($this->rename . 'LAST_EXECUTE_MRW', $time);
                    $query2 = 'UPDATE  ' . _DB_PREFIX_ . 'mrwtest SET `check_field` = 0';
                    $setCheck = Db::getInstance()->Execute($query2);
                }
            } else {
                /* No Code Executed */
        ...
        if ($this->isCarrierMRW($id_order)) {
                ...
                if (Tools::isSubmit('regNewLump')) {
                    ...
                    if ($rowBultos && isset($rowBultos) && $rowBultos['cant'] != '') {
                        ...
                        $sql_bultos = 'INSERT INTO `' . _DB_PREFIX_ . $this->name . '_mrw`
                                      (
                                           `id`,`id_shop`, `order_id`,`subscriber`,`date`,`cant`
            ...
            if (Tools::isSubmit('regNewServ')) {
                ...
                if ($this->debug_activo == '1') {
                    ...
                    , el cambio de Abonado (' . $newAbonado . ')  o el cambio de Retorno (' . $retorno_activado . ')');
                }
                ...
                if ($rowServ) {
                    ...
                    $sql_serv = 'INSERT INTO `' . _DB_PREFIX_ . $this->name . '_mrw`
                                (
                                     `id`, `id_shop`,`order_id`,`date`,`subscriber`,`cant`, `service`,
                                     `saturday`,`agency`, `backReturn`
                                )
                                     VALUES
                                (
            ...
            if ((strncmp(_PS_VERSION_, "1.4", 3) == 0) || (strncmp(_PS_VERSION_, "1.5", 3) == 0)) {
                ...
                return $output_mrw;
            //APORVINO (JORGE MIRAT): adaptar algo mejor al AdminOrders en 1.6.x. No es tampoco ideal pero sale algo mejor.
            } elseif (strncmp(_PS_VERSION_, "1.6", 3) == 0){
                    $output_mrw = '
     ...
     * @param String $type, Tipo de número de telefono, móvil o fijo
     * @return Int 1/0
     */
    public function phoneValid($phone, $type)
    {
        if ($type == 'fijo') {
            return preg_match('/^[9][0-9]{8}$/', $phone); // fijo
        } else if ($type == 'movil') {
            return preg_match('/^[6][0-9]{8}$/', $phone); // movil
        } else {
            return 0;
        }
    }
