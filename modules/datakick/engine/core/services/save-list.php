...
class SaveListService extends Service {

  public function __construct() {
    parent::__construct('save-list');
  }
  ...
  public function process($factory, $request) {
    ...
    $table = $factory->getServiceTable('lists');
    ...
    if (! $id) {
      $factory->getUser()->getPermissions()->checkCreate('lists');
      $data['user_id'] = $factory->getUser()->getId();
      $id = $connection->insert($table, $data);
    } else {
      $factory->getUser()->getPermissions()->checkEdit('lists');
      $key = "id = $id";
