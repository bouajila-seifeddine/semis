...
class InitFunction extends Func {

    public function __construct($length) {
      parent::__construct('init', 'array[any]', parent::VARIADIC, true);
      $this->length = $length;
    }
    ...
    public function validateParameters($args) {
      ...
      if ($len>0 && $len<=2) {
        if (! Types::isArray($args[0]))
            throw new \Exception("Parameter validation failed for 'head': first parameter must be an array, " . $args[0] . ' given');
        if ($len === 2 && !Types::isNumber($args[1]))
            throw new \Exception("Parameter validation failed for 'head': second optional parameter must be a number, " . $args[1] . ' given');
        return;
      }
      throw new \Exception("Parameter validation failed for 'head': parameter count: [ " . implode(', ', $args) . ' ]');
    }
...
EOE;
    ...
    public function getSqlExpression($args, $type, $argTypes, $query, Context $context) {
        ...
        return "IF($num<$len, SUBSTRING_INDEX($arr, CHAR(1), $len - $num), '')";
    }
}
