...
class MigrationVersion_1_5_0 extends MigrationVersion {
  ...
  public function migrateData($factory, $conn) {
    ...
    $this->update('executions', array('user_id' => $userId), array());
    $this->update('endpoint', array('user_id' => $userId), array());
    $this->update('schedule', array('user_id' => $userId), array());
    $this->update('xml-templates', array('user_id' => $userId, 'public' => true), array());
    $this->update('lists', array('user_id' => $userId, 'public' => true), array());
    $this->update('mass-updates', array('user_id' => $userId, 'public' => true), array());

    // encrypt passwords
    $placeConfig = $factory->getServiceTable('place-config');
    $res = $conn->query("SELECT place_id, value FROM $placeConfig WHERE name='password'");
    while ($row = $res->fetch()) {
      $conn->insert($placeConfig, array(
        'place_id' => $row['place_id'],
        'name' => 'pwd',
        'value' => $factory->getCipher()->encrypt($row['value'])
      ));
    }
    $conn->delete($placeConfig, array('name' => 'password'));
    return true;
  }
}
