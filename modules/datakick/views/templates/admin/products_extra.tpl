<input id="dksubmit" type="hidden" name="dksubmit" value="1" />
{foreach $datakickFields as $f => $fieldset}
	{block name="fieldset"}
			...
			{elseif $key == 'input'}
				...
				{foreach $field as $input}
					...
					{if $input.type == 'hidden'}
						<input type="hidden" name="{$input.name}" id="{$input.name}" value="{$datakickFieldsValue[$input.name]|escape:'html':'UTF-8'}" />
					{else}
						{block name="label"}
						...
						{block name="field"}
							...
							{if $input.type == 'text'}
								...
								{foreach $languages as $language}
									{assign var='value_text' value=$datakickFieldsValue[$input.name][$language.id_lang]}
									{if $languages|count > 1}
								...
								{/if}
								{else}
									{assign var='value_text' value=$datakickFieldsValue[$input.name]}
									{if isset($input.maxchar) || isset($input.prefix) || isset($input.suffix)}
									<div class="input-group{if isset($input.class)} {$input.class}{/if}">
									{/if}
									{if isset($input.maxchar) && $input.maxchar}
										...
										<div class="form-control-static row">
											<div class="col-xs-6">
												...
												{foreach $input.options.query AS $option}
													{if is_object($option)}
														{if !in_array($option->$input.options.id, $datakickFieldsValue[$input.name])}
													...
													{foreach $optiongroup[$input.options.options.query] as $option}
														<option value="{$option[$input.options.options.id]}"
															{if isset($input.multiple)}
																{foreach $datakickFieldsValue[$input.name] as $field_value}
																	{if $field_value == $option[$input.options.options.id]}selected="selected"{/if}
																{/foreach}
															{else}
																{if $datakickFieldsValue[$input.name] == $option[$input.options.options.id]}selected="selected"{/if}
															{/if}
												...
												{if is_object($option)}
													<option value="{$option->$input.options.id}"
														{if isset($input.multiple)}
															...
															{/foreach}
														{else}
															{if $datakickFieldsValue[$input.name] == $option->$input.options.id}
																selected="selected"
															{/if}
														{/if}
							...
							{elseif $input.type == 'radio'}
								{foreach $input.values as $value}
									<div class="radio {if isset($input.class)}{$input.class}{/if}">
										{strip}
										<label>
										<input type="radio"	name="{$input.name}" id="{$value.id}" value="{$value.value|escape:'html':'UTF-8'}"{if $datakickFieldsValue[$input.name] == $value.value} checked="checked"{/if}{if isset($input.disabled) && $input.disabled} disabled="disabled"{/if}/>
											{$value.label}
										</label>
										{/strip}
									</div>
							...
							{elseif $input.type == 'switch'}
								<span class="switch prestashop-switch fixed-width-lg">
									{foreach $input.values as $value}
									<input type="radio" name="{$input.name}"{if $value.value == 1} id="{$input.name}_on"{else} id="{$input.name}_off"{/if} value="{$value.value}"{if $datakickFieldsValue[$input.name] == $value.value} checked="checked"{/if}{if isset($input.disabled) && $input.disabled} disabled="disabled"{/if}/>
									{strip}
							...
							{elseif $input.type == 'textarea'}
								...
								{if isset($input.lang) AND $input.lang}
									{foreach $languages as $language}
										...
										{/if}
												...
												{/if}
												<textarea{if isset($input.readonly) && $input.readonly} readonly="readonly"{/if} name="{$input.name}_{$language.id_lang}" id="{if isset($input.id)}{$input.id}{else}{$input.name}{/if}_{$language.id_lang}" class="{if isset($input.autoload_rte) && $input.autoload_rte}rte autoload_rte{else}textarea-autosize{/if}{if isset($input.class)} {$input.class}{/if}"{if isset($input.maxlength) && $input.maxlength} maxlength="{$input.maxlength|intval}"{/if}{if isset($input.maxchar) && $input.maxchar} data-maxchar="{$input.maxchar|intval}"{/if}>{$datakickFieldsValue[$input.name][$language.id_lang]|escape:'html':'UTF-8'}</textarea>
										{if $languages|count > 1}
											</div>
											<div class="col-lg-2">
												<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">
								...
								{else}
									...
									{/if}
									<textarea{if isset($input.readonly) && $input.readonly} readonly="readonly"{/if} name="{$input.name}" id="{if isset($input.id)}{$input.id}{else}{$input.name}{/if}" {if isset($input.cols)}cols="{$input.cols}"{/if} {if isset($input.rows)}rows="{$input.rows}"{/if} class="{if isset($input.autoload_rte) && $input.autoload_rte}rte autoload_rte{else}textarea-autosize{/if}{if isset($input.class)} {$input.class}{/if}"{if isset($input.maxlength) && $input.maxlength} maxlength="{$input.maxlength|intval}"{/if}{if isset($input.maxchar) && $input.maxchar} data-maxchar="{$input.maxchar|intval}"{/if}>{$datakickFieldsValue[$input.name]|escape:'html':'UTF-8'}</textarea>
									{if isset($input.maxchar) && $input.maxchar}
										<script type="text/javascript">
										$(document).ready(function(){
											countDown($("#{if isset($input.id)}{$input.id}{else}{$input.name}{/if}"), $("#{if isset($input.id)}{$input.id}{else}{$input.name}{/if}_counter"));
									...
									<div class="checkbox{if isset($input.expand) && strtolower($input.expand.default) == 'show'} hidden{/if}">
										{strip}
											<label for="{$id_checkbox}">
												<input type="checkbox" name="{$id_checkbox}" id="{$id_checkbox}" class="{if isset($input.class)}{$input.class}{/if}"{if isset($value.val)} value="{$value.val|escape:'html':'UTF-8'}"{/if}{if isset($datakickFieldsValue[$id_checkbox]) && $datakickFieldsValue[$id_checkbox]} checked="checked"{/if} />
										...
										{if $key == 'months'}
											...
											*}
											{foreach $select as $k => $v}
												<option value="{$k}" {if $k == $datakickFieldsValue[$key]}selected="selected"{/if}>{l s=$v}</option>
											{/foreach}
										{else}
											{foreach $select as $v}
												<option value="{$v}" {if $v == $datakickFieldsValue[$key]}selected="selected"{/if}>{$v}</option>
											{/foreach}
										{/if}
									</select>
								</div>
								...
								<div class="col-lg-2">
									<div class="row">
										<div class="input-group">
											...
											name="{$input.name}"
											value="{$datakickFieldsValue[$input.name]|escape:'html':'UTF-8'}" />
										</div>
									</div>
								</div>
							</div>
							...
							{elseif $input.type == 'free'}
								{$datakickFieldsValue[$input.name]}
