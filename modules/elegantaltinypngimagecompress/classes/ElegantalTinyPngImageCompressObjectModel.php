...
class ElegantalTinyPngImageCompressObjectModel extends ObjectModel
{
    ...
    /**
     * Returns the static model of the calling class. The model returned is a static instance of the class.
    ...
    /**
     * Finds record from db and returns as object
     * @param int $id
     * @return class|boolean
     */
    public function loadObjectByPk($id)
    {
        // Get class of current object
        $class = get_class($this);

        // Load model object
        $model = new $class((int) $id);
        if (!Validate::isLoadedObject($model)) {
            return false;
        }

        // Get attributes from db
        $modelAttrs = $this->findByPk($id);
        if (!$modelAttrs) {
            return false;
        }

        $pk = $this->def['primary'];
        $model->{$pk} = $modelAttrs[$pk];
        ...
        foreach ($this->def['fields'] as $attr => $options) {
            if (isset($this->def['multilang']) && $this->def['multilang'] && isset($options['lang']) && $options['lang'] == true) {
                foreach ($languages as $lang) {
                    $tmpModel = $this->findByPk($id, $lang['id_lang']);
                    $model->{$attr}[$lang['id_lang']] = $tmpModel[$attr];
                }
            } else {
                $model->$attr = $modelAttrs[$attr];
            }
        }
    ...
    public function find($criteria = array())
    {
        ...
        if (!empty($criteria['order'])) {
            $orderBy = (strpos($criteria['order'], '.') === false) ? 't.' . $criteria['order'] : $criteria['order'];
            $sql->orderBy($orderBy);
        }
    ...
    protected function validateModelAttribute($field, $value, $data, $htmlentities = true, $lang = '')
    {
        ...
        if (isset($data['required']) && $data['required'] && empty($value) && $value !== '0') {
            if (!$this->id || $field != 'passwd') {
                $error = '<b>' . self::displayFieldName($field, get_class($this), $htmlentities) . ' ' . $lang . ' </b> ' . Tools::displayError('is required.');
            }
        }

        // Checking for maximum fields sizes
        if (isset($data['size']) && !empty($value) && Tools::strlen($value) > $data['size']) {
            $error = sprintf(Tools::displayError('%1$s is too long. Maximum length: %2$d'), self::displayFieldName($field, get_class($this), $htmlentities) . ' ' . $lang, $data['size']);
        }
        ...
        if (!empty($value) || $value === '0' || ($field == 'postcode' && $value == '0')) {
            if (isset($data['validate']) && !call_user_func(array('Validate', $data['validate']), $value) && (!empty($value) || $data['required'])) {
                $error = '<b>' . self::displayFieldName($field, get_class($this), $htmlentities) . ' ' . $lang . '</b> ' . Tools::displayError('is invalid.');
            }
        }
