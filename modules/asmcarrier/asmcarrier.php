...
class asmcarrier extends CarrierModule {
    ...
    */
    ...
    public function install() {
    	...
    	//preparamos la tabla para los parcels
        $query = "CREATE TABLE IF NOT EXISTS "._DB_PREFIX_."asm_parcels (
			        id int(11) NOT NULL AUTO_INCREMENT,
        ...
        if(!Db::getInstance()->Execute($query)){
        	AsmLog::error('Imposible crear la tabla '._DB_PREFIX_.'asm_parcels usando el ENGINE='._MYSQL_ENGINE_."\n\r");
	        // do rollback
	       // $this->tablesRollback();
	        return false;
	    }
        ...
        $carrierConfig = array(
				...
				4 => array('name' => 'ASM - Eurobusiness Parcel',
                        'id_tax_rules_group' => 0,
                        'active' => true,
                        'deleted' => 0,
                        'shipping_handling' => false,
                        'range_behavior' => 0,
                        'delay' => array(Language::getIsoById(Configuration::get('PS_LANG_DEFAULT')) => 'Servicio entrega UE'),
                        'id_zone' => 1,
                        'is_module' => true,
                        'shipping_external' => true,
                        'external_module_name' => $this->_moduleName,
        ...
        if (!parent::install() || !$this->registerHook('updateCarrier') || !$this->registerHook('displayCarrierList') || !$this->registerHook('displayBeforePayment'))
            return false;
    ...
    public function uninstall() {
        // Uninstall
        if (!parent::uninstall() || !$this->unregisterHook('updateCarrier') || !$this->unregisterHook('displayCarrierList') || !$this->unregisterHook('displayBeforePayment'))
            return false;
        ...
        if (Configuration::get('PS_CARRIER_DEFAULT') == (int)($Carrier1->id) ||
        	...
        	Configuration::get('PS_CARRIER_DEFAULT') == (int)($Carrier5->id)) {
            global $cookie;
        ...
        $Carrier4->deleted = 1;
        $Carrier5->deleted = 1;
        if (!$Carrier1->update() || !$Carrier2->update() || !$Carrier3->update() || !$Carrier4->update() || !$Carrier5->update())
            return false;
        // Borramos el tab ASM
        ...
        Db::getInstance()->Execute('DROP TABLE '._DB_PREFIX_.'asm_parcels');
    ...
    public static function installExternalCarrier($config) {
        ...
        if ($carrier->add()) {
			...
			if($config['name'] == 'ASM - Eurobusiness Parcel'){
            	if (!copy(dirname(__FILE__).'/asm_asm24.png', _PS_SHIP_IMG_DIR_.'/'.(int)$carrier->id.'.png'))
                	return false;
            }
            // Return ID Carrier
    ...
    */
    ...
    private function _displayForm() {
        ...
        if(!Configuration::get('ASM_URL') || Configuration::get('ASM_URL') == '') {
            $asm_wsvc = 'https://www.asmred.com/websrvs/ecm.asmx?wsdl';
        } else {
        ...
        $servicio_asm_seccionado_option_asmebp = '<option value="" title="Seleccione un servicio">Seleccione un servicio</option>';
        ...
        $servicio_asm_seccionado_selected_asmebp = '';
        ...
        foreach ($id_carrier_list as $key => $value) {
			...
			if(Tools::getValue('asm_servicio_seleccionado_asmebp', Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP')) == $value['id']) {
                $servicio_asm_seccionado_selected_asmebp = 'selected="selected"';
            } else {
                $servicio_asm_seccionado_selected_asmebp = '';
            }
            ...
            $servicio_asm_seccionado_option_asmebp .= '<option title="'.$value['name'].'" value="'.$value['id'].'" '.$servicio_asm_seccionado_selected_asmebp.'>'.$value['name'].'</option>';
        }
        ...
        $servicio_gratuito_eurobusinessparcel = "";
        $mostrar_todo_si = "";
        $mostrar_todo_no = "";
        $asm_como_modulo = "";
		...
		$envio_servicio_eurobusinessparcel_no = "";
        $envio_servicio_eurobusinessparcel_si = "";
		...
		if(Tools::getValue('asm_servicio_eurobusinessparcel', Configuration::get('ASM_EUROBUSINESSPARCEL')) == "0") {
            $envio_servicio_eurobusinessparcel_no = "checked=\"checked\"";
        }
        if(Tools::getValue('asm_servicio_eurobusinessparcel', Configuration::get('ASM_EUROBUSINESSPARCEL')) == "EUROBUSINESSPARCEL") {
            $envio_servicio_eurobusinessparcel_si = "checked=\"checked\"";
        }
		...
		$parcels_si = "";
        $parcels_no = "";
        if(Tools::getValue('asm_parcels', Configuration::get('ASM_PARCELS')) == "0") {
        	$parcels_no = "checked=\"checked\"";
        }
        if(Tools::getValue('asm_parcels', Configuration::get('ASM_PARCELS')) == "1") {
        	$parcels_si = "checked=\"checked\"";
        }
        ...
        $asm_incoterm_10 = "";
        $asm_incoterm_20 = "selected=\"selected\"";
        $asm_incoterm_30 = "";
        $asm_incoterm_40 = "";
        $asm_incoterm_50 = "";

        //AsmLog::info("incoterm: " . Tools::getValue('asm_incoterm', Configuration::get('ASM_INCOTERM')));

        if(Tools::getValue('asm_incoterm', Configuration::get('ASM_INCOTERM')) == "10") {
            $asm_incoterm_10 = "selected=\"selected\"";
            $asm_incoterm_20 = "";
            $asm_incoterm_30 = "";
            $asm_incoterm_40 = "";
            $asm_incoterm_50 = "";
        }
        if(Tools::getValue('asm_incoterm', Configuration::get('ASM_INCOTERM')) == "20") {
            $asm_incoterm_10 = "";
            $asm_incoterm_20 = "selected=\"selected\"";
            $asm_incoterm_30 = "";
            $asm_incoterm_40 = "";
        ...
        if(Tools::getValue('asm_incoterm', Configuration::get('ASM_INCOTERM')) == "30") {
            $asm_incoterm_10 = "";
            $asm_incoterm_20 = "";
            $asm_incoterm_30 = "selected=\"selected\"";
            $asm_incoterm_40 = "";
            $asm_incoterm_50 = "";
        }
        if(Tools::getValue('asm_incoterm', Configuration::get('ASM_INCOTERM')) == "40") {
            $asm_incoterm_10 = "";
            $asm_incoterm_20 = "";
            $asm_incoterm_30 = "";
            $asm_incoterm_40 = "selected=\"selected\"";
            $asm_incoterm_50 = "";
        }
        if(Tools::getValue('asm_incoterm', Configuration::get('ASM_INCOTERM')) == "50") {
            $asm_incoterm_10 = "";
            $asm_incoterm_20 = "";
            $asm_incoterm_30 = "";
            $asm_incoterm_40 = "";
            $asm_incoterm_50 = "selected=\"selected\"";
        }
			...
			<script language="javascript">
			    ...
			    $(document).ready(function(event) {
			        if ($("#asm_prestamodulo").is(":checked")) {
						...
						$("#asm_service_5").css("display","");
			        } else {
						$("#asm_servicio_seleccionado_asm10").val("");
						$("#asm_servicio_seleccionado_asm14").val("");
						$("#asm_servicio_seleccionado_asm24").val("");
						$("#asm_servicio_seleccionado_asmeco").val("");
						$("#asm_servicio_seleccionado_asmebp").val("");
						$("#asm_service_1").css("display","none");
                        $("#asm_service_2").css("display","none");
                        $("#asm_service_3").css("display","none");
                        $("#asm_service_4").css("display","none");
						$("#asm_service_5").css("display","none");
					}
			        ...
			        $("#asm_prestamodulo").click(function (event) {
                        ...
                        $("#asm_service_5").css("display",styleTwoStatus);
						...
						<h4>'.$this->l('General configuration').' :</h4>
                                                <table style="border: 0px;" id="configuracionASM">
                                                    <tr class="asm_module_off">
                                                        ...
                                                        <td class="columna2">
                                                            <input type="hidden" name="asm_prestamodulo"  value="0" />
													...
													<tr class="asm_module_off" id="asm_service_5">
                                                        <td class="columna1"></td>
                                                        <td class="columna2">
                                                            '.$this->l('Servicio EUROBUSINESS PARCEL').' :
                                                            <select id="asm_servicio_seleccionado_asmebp" name="asm_servicio_seleccionado_asmebp">
                                                                '.$servicio_asm_seccionado_option_asmebp.'
                                                            </select>
                                                    ...
                                                    <tr class="asm_module_off">
                                                        ...
                                                        <td class="columna2">
                                                            ...
                                                            <p class="tip">URL del servicio web de ASM. Por defecto: https://www.asmred.com/websrvs/ecm.asmx?wsdl</p>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                    	<td colspan="2"><hr/></td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="columna2">
                                                            <select name="asm_servicio_envio_gratuito">
                                                              ...
                                                              <option '.$servicio_gratuito_eurobusinessparcel.' value="EUROBUSINESSPARCEL">EUROBUSINESSPARCEL</option>
                                                            </select>
                                                    ...
                                                    <tr>
													 ...
													 <tr>
                                                        <td class="columna1">'.$this->l('Habilitar servicio EUROBUSINESS PARCEL').' : </td>
                                                        <td class="columna2">
                                                            <input type="radio" name="asm_servicio_eurobusinessparcel" value="0" '.$envio_servicio_eurobusinessparcel_no.'/>No&nbsp;&nbsp;&nbsp;
                                                            <input type="radio" name="asm_servicio_eurobusinessparcel" value="EUROBUSINESSPARCEL" '.$envio_servicio_eurobusinessparcel_si.'/>Si
                                                            <p class="tip">Habilita como método de envío el servicio EUROBUSINESSPARCEL</p>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                    	<td colspan="2"><hr/></td>
                                                    ...
                                                    <tr class="asm_module_off">
                                                        <td class="columna1">'.$this->l('Incoterm').' : </td>
                                                        <td class="columna2">
                                                            <select name="asm_incoterm">
                                                              <option '.$asm_incoterm_10.' value="10">Incoterm 10 DDP. COSTES REMITENTE: transporte,despacho,aranceles, impuestos. COSTES DESTINATARIO: no tiene costes</option>
                                                              <option '.$asm_incoterm_20.' value="20">Incoterm 20 DAP. COSTES REMITENTE: transporte. COSTES DESTINATARIO: despacho, aranceles e impuestos</option>
                                                              <option '.$asm_incoterm_30.' value="30">Incoterm 30 DDP, I.V.A. no pagado . COSTES REMITENTE: transporte,despacho, aranceles. COSTES DESTINATARIO: impuestos</option>
                                                              <option '.$asm_incoterm_40.' value="40">Incoterm 40 DAP, despachado. COSTES REMITENTE: transporte, despacho. COSTES DESTINATARIO: aranceles, impuestos</option>
                                                              <option '.$asm_incoterm_50.' value="50">Incoterm 50c DDP, bajo valor . COSTES REMITENTE: transporte, despacho. COSTES DESTINATARIO: no tiene costes</option>
                                                            </select>
                                                            <p class="tip">Incoterm que será usado en caso de envíos destino fuera de la UE con el servicio Eurobusiness Parcel. <br /><b>No tiene efecto para envíos nacionales e internacionales destino UE.</b></p>
                                                        </td>
                                                    </tr>
													<tr>
                                                    	<td colspan="2"><hr/></td>
                                                    ...
                                                    <tr class="asm_module_off">
                                                        <td class="columna1">'.$this->l('Número de bultos (fijo)').' : </td>
                                                        <td class="columna2">
                                                            ...
                                                            <p class="tip">Indique el número de bultos.</p>
                                                        </td>
                                                    </tr>
                                                    <tr class="asm_module_off">
													...
													<tr class="asm_module_off" >
                                                        <td class="columna1">'.$this->l('Puntos de recogida').' :</td>
                                                        <td class="columna2">
                                                            <input type="radio" name="asm_parcels" value="0" '.$parcels_no.'/>No&nbsp;&nbsp;&nbsp;
                                                            <input type="radio" name="asm_parcels" value="1" '.$parcels_si.'/>Si
                                                            <p class="tip">Habilitar la posibilidad de seleccionar puntos de recogida.</p>
                                                        </td>
                                                    </tr>
                                                    <tr class="asm_module_off">
    ...
    private function _postValidation() {
        ...
        if(Tools::getValue('asm_prestamodulo') && Tools::getValue('asm_prestamodulo') == "1" ) {
            if(Tools::getValue('asm_servicio_seleccionado_asm10') == '' && Tools::getValue('asm_servicio_seleccionado_asm14') == '' && Tools::getValue('asm_servicio_seleccionado_asm24') == '' && Tools::getValue('asm_servicio_seleccionado_asmeco') == '' && Tools::getValue('asm_servicio_seleccionado_asmebp') == '') {
                echo "Error 0x0008234";
    ...
    private function _postProcess() {
        ...
        if (Configuration::updateValue('ASM_PRESTAMODULO', Tools::getValue('asm_prestamodulo')) &&
            ...
            Configuration::updateValue('ASM_SERVICIO_SELECCIONADO_ASMEBP', Tools::getValue('asm_servicio_seleccionado_asmebp')) &&
            ...
            Configuration::updateValue('ASM_EUROBUSINESSPARCEL', Tools::getValue('asm_servicio_eurobusinessparcel')) &&
        	...
        	Configuration::updateValue('ASM_PARCELS', Tools::getValue('asm_parcels')) &&
			...
			Configuration::updateValue('ASM_INCOTERM', Tools::getValue('asm_incoterm')) &&
		...
		if(!isset($generarEnvioHook) || empty($generarEnvioHook)) {
            ...
            if($this->isCarrierASM($id_order) || ($this->isASMModule() && $this->isCarrierASMNonModule($orderTemp->id_carrier))) {
				if($valida) {
					...
					if($rowCarrier['codigo_envio']!=NULL) {
						...
						if (file_exists(_PS_ROOT_DIR_."/modules/asmcarrier/PDF/etiqueta_".$rowCarrier['id_envio_order'].".pdf")) {
                            ...
                            $path_download_html = 'https://www.asmred.com/Extranet/public/ecmLabel.aspx?codbarras='. $rowCarrier['codigo_envio'] . '&uid=' .Configuration::get('ASM_GUID');
							...
							AND c.id_carrier=o.id_carrier');
								if ($datosCarrier[0]['name'] == 'ASM - Eurobusiness Parcel'
									|| Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP') == $datosCarrier[0]['id_carrier']){
                        ...
                        $smarty->assign('bultos_info_b', 'Si desea emplear la <strong>configuración que ha predefinido</strong> en el módulo de ASM <strong>deje el campo "Bultos" vacío.</strong> <br>  <strong>Eurobusiness Parcel</strong> sólo permite envíos <strong>monobulto, sin retorno y sin reembolso.</strong>');
				...
				if($valida) {
                    ...
                    $path_download_html = 'https://www.asmred.com/Extranet/public/ecmLabel.aspx?codbarras='. $asm_track_value['codigo_envio'] . '&uid=' .Configuration::get('ASM_GUID');
                    ...
                    if($_SESSION["ultimoErrorASM"] ==  "") {
                        ...
                        $smarty->assign('testval', print_r($asm_track_value,true));
						...
						if(!$this->isASMModule()) {
							...
							if(Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP') == $datosCarrier[0]['id_carrier']){
								$isEurobusinessparcel = true;
							}
						}
    ...
    */
	...
	function isCarrierASM($id_order) {
		...
		if(in_array($rowCarrier['id_carrier'], $arrST)
			...
			|| in_array($rowCarrier['id_carrier'], $arrSTD)) {
			return true;
		}
		else { return false; }
	}
    ...
    function isCarrierASMNonModule ($id_carrier) {
        if (Configuration::get('ASM_SERVICIO_SELECCIONADO_ASM10') == $id_carrier || 
			Configuration::get('ASM_SERVICIO_SELECCIONADO_ASM14') == $id_carrier || 
			Configuration::get('ASM_SERVICIO_SELECCIONADO_ASM24') == $id_carrier || 
			Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMECO') == $id_carrier || 
			Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP') == $id_carrier) {
            return true;
        } else {
            return false;
        }
	...
	function isCarrierASMModule ($id_carrier) {
		if (Configuration::get('MYCARRIER1_CARRIER_ID') == $id_carrier ||
			Configuration::get('MYCARRIER2_CARRIER_ID') == $id_carrier ||
			Configuration::get('MYCARRIER3_CARRIER_ID') == $id_carrier ||
			Configuration::get('MYCARRIER4_CARRIER_ID') == $id_carrier ||
			Configuration::get('MYCARRIER5_CARRIER_ID') == $id_carrier) {
			return true;
		} else {
			return false;
		}
    ...
    */
    ...
    public function getOrderShippingCost($params, $shipping_cost) {
		...
		// Albert a 30/12/2016. Reparación AD
		if ($usuario_pais == 'AD') {
		$usuario_cp = str_replace("AD", "", $usuario_cp);
		}
        ...
        */
        ...
        if ($this->id_carrier == (int)(Configuration::get('MYCARRIER1_CARRIER_ID')) && ($usuario_pais=="ES" || $usuario_pais=="PT")){
            ...
            if(Configuration::get('ASM_ASM10')){
                ...
                if(Configuration::get('ASM_COSTE_FIJO_ENVIO')){
					...
					// Albert a 02/01/2017 se excluye el transportista si el importe del carrito es negativo
					if ((float) $subimporte < 0) return false;
                    /*
        ...
        // Albert a 30/12/2016. Reparación AD. Andorra solo tiene ASM24, se hace solo aquí
        ...
        // Filtramos para EUROBUSINESSPARCEL
        if ($this->id_carrier == (int)(Configuration::get('MYCARRIER5_CARRIER_ID')) && ($usuario_pais!="ES" && $usuario_pais!="PT")){
            $servicio = 54;
            $horario  = 3;
            //AsmLog::info("Filtramos por EUROBUSINESSPARCEL\n\r");
           
        	if(Configuration::get('ASM_ENVIO_GRAT') && $total >= Configuration::get('ASM_IMP_MIN_ENVIO_GRA') && Configuration::get('ASM_IMP_MIN_ENVIO_GRA') != ""){
        		if(Configuration::get('ASM_SERVICIO_GRAT') == 'EUROBUSINESSPARCEL'){
        			return 0;
        		}
        	}
            // No es gratuito
            if(Configuration::get('ASM_EUROBUSINESSPARCEL')){
                $importe = 0;
                $subimporte=0;
                ...
                if(Configuration::get('ASM_COSTE_FIJO_ENVIO')){
                    ...
                    //necesitamos el tipo_servicio, cp_cliente, pais y peso
						...
						$codPaises = array('AD'=> 376,
						 ...
						 if(!empty($codPaises[$paisOrig]) && !empty($codPaises[$paisDest])) {
						   $coste_envio = $this->valora($servicio,$horario,$bultos,$peso,$cpOrig,$cpDest,$codPaises[$paisOrig],$codPaises[$paisDest]);
						} else {
						
						  AsmLog::info("\n\rEnvio europa : ".$paisOrig." (".$codPaises[$paisOrig].") -> ".$paisDest." (".$codPaises[$paisDest].") \n\r");
						  return false;
						}
    	...
    	for($i=1;$i<$max;$i++){
  			if($tarifas[$i]['servicio'] == $servicio){
  				if($tarifas[$i]['pais'] == $pais){
  					$cp_origen=intval($tarifas[$i]['cp_origen']);
  					$cp_destino=intval($tarifas[$i]['cp_destino']);
					$imp_negativo=intval($tarifas[$i]['importe']);
					if($cp >= $cp_origen){
						if($cp <= $cp_destino){
							$encontrado = 1;
						}
    ...
    function pedidosTabla(){
        ...
        if(Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP') != '') $pedidosNoModule .= 'OR c.id_carrier = '.Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP').' ';
    ...
    function imprimirEtiquetas($id_pedido=0, $adminOrders=false) {
        ...
        if(!$hay_track){
            ...
            if(version_compare(_PS_VERSION_, '1.5') >= 0) {
                $select = 'SELECT o.id_order,o.id_cart,o.module,o.total_paid_real,c.name,u.email,a.firstname,
            ...
            if(!$this->isASMModule()) {
                ...
                switch($datos[0]['name']){
                      ...
                      case 'ASM - Servicio ECONOMY':
                        ...
                        $horario  =16;
                      break;
					  case 'ASM - Eurobusiness Parcel':
                        $asm_tipo_servicio = 'EUROBUSINESSPARCEL';
                        $servicio =74;
                        $horario  =3;
                      break;
                }
				...
				if(Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP') == $datos[0]['id_carrier']){
                    $asm_tipo_servicio = 'EUROBUSINESSPARCEL';
                    $servicio =74;
                    $horario  =3;
                }
			...
			$retorno = intval(Tools::getValue('asm_retorno'));
			...
			// Parcels
			$parcels = Db::getInstance()->ExecuteS('SELECT * FROM '._DB_PREFIX_.'asm_parcels where id_cart = '.$datos[0]['id_cart']);
            ...
            //else if($asm_pais == 'AD') {
            //   $asm_pais = '34';
            //   $asm_CP_destinatario = '87111';
              //}

            //HAY QUE CONTROLAR SI EL COMPRADOR A ELEGIDO CONTRAREEMBOLSO Y PONERLO EN EL PARAMETRO
            $metodo_pago = $datos[0]['module'];
            if($metodo_pago == 'cashondelivery' || $metodo_pago == 'megareembolso' || $metodo_pago == 'cashondeliverywithfee' || $metodo_pago == 'maofree_cashondeliveryfee' || $metodo_pago == 'codfee' || $metodo_pago == 'cashondeliveryplus' ){
            ...
            // Albert a 03/04/2017.
            // Como los clientes ya tienen en la Config. del módulo el http, forzamos aquí el https para que no tengan que cambiarlo ellos.
            // Además de toda la inclusión del https.
			...
			// Albert a 02/01/2017. Lleno siempre los dos teléfonos. Hay servicios como EE y EBP que mejor que estén llenos.
			if ($asm_telefono_destinatario == '') { $asm_telefono_destinatario = $asm_movil_destinatario;    }
			if ($asm_movil_destinatario    == '') { $asm_movil_destinatario    = $asm_telefono_destinatario; }			

            // Referencia DEMO para saltarnos el error -70
            $asm_referencia2 = $asm_referencia + rand(10,100);

            //Dani a 29/12/2016
            $incoterm = Configuration::get('ASM_INCOTERM');

            //AsmLog::info("\n\rIncoterm:&nbsp;".$incoterm."\n\r");
            //AsmLog::info("Servicio:&nbsp;".$servicio."\n\r");
            //AsmLog::info("Peso:&nbsp;".$asm_peso_origen."\n\r");

            //Dani a 29/12/2016 Si el servicio es EBP y el peso es inferior a 3, entonces lo cambio a SEBP
            if($servicio==74 && $asm_peso_origen < 3) $servicio=76;
            ...
            $XML=
                ...
                <Servicios uidcliente="' . $uidCliente . '">
                  <Envio>
                    ...
                    // Albert a 30/12/2016. Reparación AD.
					$CpAndorra=$parcels[0]['cp'];
					//if($asm_pais == 'AD') $CpAndorra='87111';
		...
		if (!empty($parcels[0])) {
                $XML.='<Direccion><![CDATA[' . $parcels[0]['direccion'] . ']]></Direccion>
                      ...
                      <Observaciones><![CDATA[' . $parcels[0]['nombre'] . ']]></Observaciones>
                      <Email>' . $asm_email_destinatario . '</Email>
		      <Codigo>' . $parcels[0]['codigo'] . '</Codigo>';
		}else{
                $XML.='<Direccion><![CDATA[' . $asm_nombre_via_destinatario . ']]></Direccion>
            ...
            curl_setopt($ch, CURLOPT_FORBID_REUSE, TRUE);
            curl_setopt($ch, CURLOPT_FRESH_CONNECT, TRUE);
            curl_setopt($ch, CURLOPT_URL, "https://www.asmred.com/websrvs/printserver.asmx");
            curl_setopt($ch, CURLOPT_POSTFIELDS, $XML);
            curl_setopt($ch, CURLOPT_HTTPHEADER, Array("Content-Type: text/xml; charset=UTF-8"));
			...
			if ($asm_tipo_servicio == 'EUROBUSINESSPARCEL'){
				$isEurobusinessparcel = true;
			}
        } else{
    ...
    function guardarEnvio2($id_order,$codigoBarras,$num_albaran,$codigo_barras,$cp_destino)
    {
        ...
        $enlace='https://www.asmred.com/Extranet/Public/ExpedicionASM.aspx?cpDst='. $cp_destino .'&codigo='.$codigoBarras;
    ...
    function inicializarAsmEnvios()
    {
        ...
        if(!$this->isASMModule()) {
            ...
            if(Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP') != '') $pedidosNoModule .= 'OR o.id_carrier LIKE '.Configuration::get('ASM_SERVICIO_SELECCIONADO_ASMEBP').' ';
        ...
        foreach ($envios as $envio){
        	if(!Db::getInstance()->ExecuteS('SELECT id_envio_order FROM '._DB_PREFIX_.'asm_envios where id_envio_order = "'.$envio['id_order'].'"')){
				Db::getInstance()->ExecuteS('INSERT INTO '._DB_PREFIX_.'asm_envios (id_envio_order,codigo_envio,url_track,num_albaran,fecha) VALUES ("'.$envio['id_order'].'","","","","'.date('Y-m-d H:i:s').'")');		
			}
        }
        return true;
    }
...
public function hookDisplayCarrierList($params)
    {
	global $smarty;
	$showParcels = Configuration::get('ASM_PARCELS');
	$selectedCarrier = $params['cart']->id_carrier;
	
	if ($showParcels == 1 && (
			(!$this->isASMModule() && $this->isCarrierASMModule($selectedCarrier)) ||
			($this->isASMModule() && $this->isCarrierASMNonModule($selectedCarrier))
		)) {
		$smarty->assign('postalcode',$params['address']->postcode);
		return $this->display(__FILE__, 'templates/selectPickup.tpl');
	}
}
public function hookDisplayBeforePayment($params)
{
	$parcel = Tools::getValue('parcel');
	if (!empty($parcel['codigo']))
		Db::getInstance()->ExecuteS('INSERT INTO '._DB_PREFIX_.'asm_parcels (id_cart,codigo,nombre,direccion,cp,localidad) 
									VALUES ("'.$params['cart']->id.'","'.$parcel['codigo'].'","'.$parcel['nombre'].'","'.$parcel['direccion'].'","'.$parcel['cp'].'","'.$parcel['localidad'].'")');		
