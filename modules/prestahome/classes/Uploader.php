...
class FileUpload
{
    ...
    public function __construct($uploadName = null)
    {
        if ($uploadName !== null) {
        ...
        } elseif (isset($_SERVER['HTTP_X_FILE_NAME']) || Tools::getIsset($this->uploadName)) {
            $this->isXhr = true;

            $this->fileName = isset($_SERVER['HTTP_X_FILE_NAME']) ?
                                    $_SERVER['HTTP_X_FILE_NAME'] : Tools::getValue($this->uploadName);
        ...
        if ($this->fileName) {
            ...
            if (array_key_exists('extension', $pathinfo) && array_key_exists('filename', $pathinfo)) {
                $this->fileExtension = Tools::strtolower($pathinfo['extension']);
    ...
    private function checkExtension($ext, $allowedExtensions)
    {
        ...
        if (!in_array(Tools::strtolower($ext), array_map('strtolower', $allowedExtensions))) {
            return false;
        }
    ...
    private function fixDir($dir)
    {
        ...
        $dir = str_replace('\\', $slash, $dir);
        return Tools::substr($dir, -1) == $slash ? $dir : $dir . $slash;
    }
    ...
    private function jsMatcher($matches)
    {
        ...
        return sprintf('\\u%04s', Tools::strtoupper(bin2hex($chr)));
    }
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null)
    {
        ...
        if (!empty($this->newFileName)) {
            $this->fileName = $this->newFileName.'.'.$this->fileExtension;
            $this->savedFile = $this->uploadDir . $this->fileName;
