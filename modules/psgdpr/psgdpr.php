...
class Psgdpr extends Module
{
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            if ($this->ps_version) { // if 1.7
                if ($this->registerHook('additionalCustomerFormFields') && $this->registerHook('validateCustomerFormFields')) {
                    return true;
                }
            } else { // if 1.6
                if ($this->registerHook('actionBeforeSubmitAccount') && $this->registerHook('createAccountForm') && $this->registerHook('displayCustomerIdentityForm')) {
                    return true;
                }
            }
    ...
    public function hookDisplayCustomerIdentityForm()
    {
        ...
        $frontController = Context::getContext()->link->getModuleLink($this->name, 'FrontAjaxGdpr', array(), true);
        $active = Configuration::get('PSGDPR_CUSTOMER_FORM_SWITCH');
        ...
        $this->context->smarty->assign(array(
            ...
            'psgdpr_front_controller' => $frontController,
        ));

        return $this->display(dirname(__FILE__), '/views/templates/hook/consentCheckbox.tpl');
    }
    ...
    public function loadRegisteredModules()
    {
        ...
        foreach ($modules as $module) {
            ...
            $module['logoPath'] = Tools::getHttpHost(true).'/modules/'.$Module->name.'/logo.png';
