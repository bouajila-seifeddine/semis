...
{if $tab.form.tab_name != 'save_tab'}<li {if $tab.form.tab_name == 'main_tab'}class="active"{/if}><a class="list-group-item" href="#{$tab.form.tab_name|escape:'htmlall':'UTF-8'}">{$tab.form.legend.title|escape:'htmlall':'UTF-8'}</a></li>{/if}
{/foreach}
<ul>
</div>
</div>
...
<div class="tab-pane-wrapper {if $fieldset.form.tab_name == 'main_tab'}active{/if}" id="{$fieldset.form.tab_name|escape:'htmlall':'UTF-8'}">
	{*HTML CONTENT*} {if isset($fieldset.form.assigned_list)}<div class="tab-pane clearfix">{$fieldset.form.assigned_list}</div>{/if}
<div class="tab-pane">
{/if}
{$smarty.block.parent}
...
{if isset($input.preffix_wrapper)}<div id="{$input.preffix_wrapper|escape:'htmlall':'UTF-8'}" {if isset($input.wrapper_hidden) && $input.wrapper_hidden} class="hidden clearfix"{/if}>{/if}
{if isset($input.accordion_wrapper)}<a class="btn btn-primary menu-collapse-expand" data-toggle="collapse" href="#{$input.accordion_wrapper|escape:'htmlall':'UTF-8'}" aria-expanded="false" aria-controls="{$input.accordion_wrapper|escape:'htmlall':'UTF-8'}">{l s='Expand submenu optional design options' mod='iqitmegamenu'} 
 <i class="icon-angle-double-down"></i> </a><div id="{$input.accordion_wrapper|escape:'htmlall':'UTF-8'}" class="collapse collapse-menu-expand">{/if}
...
{block name="input"}
	...
	{*HTML CONTENT*}  <input type="hidden" name="submenu-elements" id="submenu-elements" value="{$submenu_content}">
	...
	<p> <input id="{$input.name|escape:'htmlall':'UTF-8'}" type="text" name="{$input.name|escape:'htmlall':'UTF-8'}" value="{$fields_value[$input.name]|escape:'html':'UTF-8'}"> </p>
	 <a href="filemanager/dialog.php?type=1&field_id={$input.name|escape:'htmlall':'UTF-8'}" class="btn btn-default iframe-upload"  data-input-name="{$input.name|escape:'htmlall':'UTF-8'}" type="button">{l s='Select image' mod='iqitmegamenu'} <i class="icon-angle-right"></i></a>
	...
	{*HTML CONTENT*} {$input.choices}

	<script>
	$("#{$input.name|escape:'htmlall':'UTF-8'} option").filter(function() {

    return $(this).val() == '{$fields_value[$input.name]|escape:'htmlall':'UTF-8'}'; 
	}).prop('selected', true);
	</script>
	{elseif $input.type == 'icon_selector'}
	<div class="input-group col-lg-3">
            <input type="text" name="{$input.name|escape:'htmlall':'UTF-8'}" class="icp icp-auto" id="{$input.name|escape:'htmlall':'UTF-8'}" value="{$fields_value[$input.name]|escape:'html':'UTF-8'}">
	...
	<div class="col-xs-2">
	<select name="{$input.name|escape:'htmlall':'UTF-8'}_type" id="{$input.name|escape:'htmlall':'UTF-8'}_type">
	...
	<div class="col-xs-2">
	<select name="{$input.name|escape:'htmlall':'UTF-8'}_width" id="{$input.name|escape:'htmlall':'UTF-8'}_width">
		{for $i=1 to 10}
  				  <option value="{$i|escape:'htmlall':'UTF-8'}" {if $fields_value[$input.name].width == $i}selected{/if}>{$i|escape:'htmlall':'UTF-8'} px</option>
		{/for}
	</select>
	</div>		
	<div class="col-xs-2">
	...
	<input type="color" data-hex="true" class="color mColorPickerInput"	name="{$input.name|escape:'htmlall':'UTF-8'}_color" value="{$fields_value[$input.name].color|escape:'html':'UTF-8'}" />
	</div>	</div>	</div>						
