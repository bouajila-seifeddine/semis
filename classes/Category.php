...
class CategoryCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'is_seed' =>            array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'is_vaper' =>            array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
...
* 	 * @param bool $method_to_create a category
    ...
    */
    ...
    public function existsInShop($id_shop)
    {
     ...
     public function getProductsCustom($id_lang, $p, $n, $order_by = null, $order_way = null, $get_total = false, $active = true, $random = false, $random_number_products = 1, $check_access = true, Context $context = null)
    {
        if (!$context) {
            $context = Context::getContext();
        }
        ...
        $sql = 'SELECT DISTINCT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) AS quantity'.(Combination::isFeatureActive() ? ', IFNULL(product_attribute_shop.id_product_attribute, 0) AS id_product_attribute,
                ...
                LEFT JOIN `'._DB_PREFIX_.'manufacturer` m
                    ON m.`id_manufacturer` = p.`id_manufacturer`
                WHERE product_shop.`id_shop` = '.(int)$context->shop->id.'
                AND p.`id_product` in ('.$productIds.') ORDER BY p.`id_product` DESC';
