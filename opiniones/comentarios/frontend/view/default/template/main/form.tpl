<div id="cmtx_form_container" class="cmtx_form_container cmtx_clear">
	<h3 class="cmtx_form_heading"><?php echo $lang_heading_form; ?>
</h3>
	...
	<?php if ($display_form) { ?>
		...
		<form id="cmtx_form" class="cmtx_form">
			...
			<?php if ($enabled_rating) { ?>
				<div class="cmtx_row cmtx_rating_row cmtx_clear "> 
				<div class="col-md-6 colvari"><label for="variedad">¿Que variedad de este banco has probado ?</label>
						<select name="variedad" id="variedad" class="cmtx_field cmtx_text_field cmtx_name_field">
								<?php
								foreach($test as $key => $value):
								echo '<option value="'.$value['id_product'].'">'.$value['name'].'</option>'; //close your tags!!
								endforeach;
								?>
						</select></div>
					<div class="col-md-6 colrate"><label for="rating">¿Que puntuación le das?</label>
			...
			<?php if ($securimage) { ?>
				<div class="cmtx_row cmtx_securimage_row cmtx_clear">
					<div class="cmtx_col_12">
