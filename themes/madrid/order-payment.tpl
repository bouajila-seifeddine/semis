...
{if !$opc}
	...
	<h1 class="pull-left">{l s='Please choose your payment method'}</h1>
	{if !isset($empty) && !$PS_CATALOG_MODE}
		<span class="heading-counter">{l s='Your shopping cart contains:'}
			<span id="summary_products_quantity">{$productNumber}</span> {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}
		</span>
	{/if}
{else}
		...
		<div class="paiement_block">
			<div id="HOOK_TOP_PAYMENT">{$HOOK_TOP_PAYMENT}</div>
				{if $HOOK_PAYMENT}
					...
					<div class="summary-shopping col-md-4 col-sm-7 col-xs-12 pull-right">
						<div class="row">
							<div class="summary-bg">
								...
								<table>
									{if $use_taxes}
										{if $priceDisplay}
											<tr>
												<td colspan="4">{if $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Total products'}{/if}</td>
												<td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td>
											</tr>
										{else}
											<tr>
												<td colspan="4">{if $display_tax_label}{l s='Total products (tax incl.)'}{else}{l s='Total products'}{/if}</td>
										...
										{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
											{if $priceDisplay}
												<tr class="cart_total_delivery" {if $shippingCost <= 0} style="display:none"{/if}>
													<td colspan="4">{if $display_tax_label}{l s='Total shipping (tax excl.)'}{else}{l s='Total shipping'}{/if}</td>
													<td colspan="2" class="price" id="total_shipping">{displayPrice price=$shippingCostTaxExc}</td>
												</tr>
											{else}
												<tr class="cart_total_delivery"{if $shippingCost <= 0} style="display:none"{/if}>
													<td colspan="4">{if $display_tax_label}{l s='Total shipping (tax incl.)'}{else}{l s='Total shipping'}{/if}</td>
									...
									{if $use_taxes}
										{if $total_tax != 0 && $show_taxes}
											...
											<tr class="cart_total_price">
												<td colspan="4">{if $display_tax_label}{l s='Total (tax excl.)'}{else}{l s='Total'}{/if}</td>
												<td colspan="2" class="price" id="total_price_without_tax">{displayPrice price=$total_price_without_tax}</td>
											</tr>
											{/if}
											<tr class="cart_total_tax">
												<td colspan="4">{l s='Tax'}</td>
												<td colspan="2" class="price" id="total_tax" >{displayPrice price=$total_tax}</td>
											</tr>
										{/if}
										<tr class="cart_total_price">
											<td colspan="4" class="total_price_container"><span>{l s='Total'}</span></td>
									...
									<tr class="cart_total_price">
										...
										{/if}
										<td colspan="{if !$voucherAllowed}4{else}2{/if}" class="total_price_container">
											<span>{l s='Total'}</span>
										</td>
				...
				{if !$opc}
					<p class="cart_navigation clearfix">
						<a href="{$link->getPageLink('order', true, NULL, "step=2")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button">
