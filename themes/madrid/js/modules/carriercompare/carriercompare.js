	...
	var query = $.ajax({
		...
		success: function(json) {
			if (json.length)
			{
				...
				$('#compare_shipping_form #id_state').selectBox('detach');
				$('#compare_shipping_form #id_state').selectBox({
	...
	$('#availableCarriers').slideUp('normal', function(){		
		...
		var query = $.ajax({
			...
			success: function(json) {
				...
				$('#compare_shipping_form #id_country').selectBox('detach');
				$('#compare_shipping_form #id_country').selectBox({
					menuTransition: 'slide',
					menuSpeed: 'fast',
					keepInViewport: false
				});
