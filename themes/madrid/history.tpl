...
<h1 class="bottom-indent">{l s='Order history'}</h1>
				...
				{foreach from=$orders item=order name=myLoop}
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						...
						<td class="history_detail">
							<a class="button button-small" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}');">
								<span>
									{l s='Details'} <i class="icon icon-chevron-right right"></i>
								</span>
							</a>
							{if isset($opc) && $opc}
...
<ul class="footer_links clearfix">
	<li>
		<a class="button button-small" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
			<span>
	...
	<li>
		<a class="button button-small" href="{$base_dir}">
			<span>{l s='Home'}</span>
		</a>
	</li>
</ul>
