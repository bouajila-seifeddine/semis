...
<div class="box box-small clearfix">
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
			...
			<input type="hidden" value="" name="submitReorder"/>
				...
				<a href="#" onclick="$(this).closest('form').submit(); return false;" class="button pull-right"><span>{l s='Reorder'}<i class="icon icon-chevron-right right"></i></span></a>
			<p class="dark">
...
<div class="adresses_bloc">
	<div class="row">
		<div class="col-xs-12 col-sm-6"{if $order->isVirtual()} style="display:none;"{/if}>
			<ul class="nolist address alternate_item box">
				<li><h3 class="page-heading"><span>{l s='Delivery address'} ({$address_delivery->alias})</span></h3></li>
		...
		<div class="col-xs-12 col-sm-6">
			<ul class="nolist address item {if $order->isVirtual()}full_width{/if} box">
				<li><h3 class="page-heading"><span>{l s='Invoice address'} ({$address_invoice->alias})</span></h3></li>
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						<td>
						<input class="order_qte_input grey"  name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$product.customizationQuantityTotal|intval}" />
							<div class="clearfix return_quantity_buttons">
								<a href="#" class="return_quantity_down button-minus"><span><i class="icon icon-minus"></i></span></a>
								<a href="#" class="return_quantity_up button-plus"><span><i class="icon icon-plus"></i></span></a>
							</div>
					...
					{foreach $product.customizedDatas  as $customizationPerAddress}
						...
						<tr class="alternate_item">
							...
							<td>
								<input class="order_qte_input grey" name="customization_qty_input[{$customizationId|intval}]" type="text" size="2" value="{$customization.quantity|intval}" />
								<div class="clearfix return_quantity_buttons">
									<a href="#" class="return_quantity_down button-minus"><span><i class="icon icon-minus"></i></span></a>
									<a href="#" class="return_quantity_up button-plus"><span><i class="icon icon-plus"></i></span></a>
								</div>
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td class="return_quantity">
							<input class="order_qte_input grey" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" />
							<div class="clearfix return_quantity_buttons">
								<a href="#" class="return_quantity_down button-minus"><span><i class="icon icon-minus"></i></span></a>
								<a href="#" class="return_quantity_up btn button-plus"><span><i class="icon icon-plus"></i></span></a>
							</div>
...
{if $return_allowed}
	<div id="returnOrderMessage">
		<h3 class="bottom-indent">{l s='Merchandise return'}</h3>
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="button button-small"><span>{l s='Make an RMA slip'}<i class="icon icon-chevron-right"></i></span></button>
...
{if !$is_guest}
	{if count($messages)}
	<h3 class="page-heading"><span>{l s='Messages'}</span></h3>
	 <div class="table_block">
		<table class="detail_step_by_step table table-bordered">
			<thead>
				<tr>
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		<h3 class="page-heading bottom-indent"><span>{l s='Add a message'}</span></h3>
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button button-medium"><span>{l s='Send'} <i class="icon icon-chevron-right"></i></span></button>
		</div>
	</form>
{else}
<p class="alert alert-info"><i class="icon icon-info-sign"></i>{l s='You cannot return merchandise with a guest account'}</p>
{/if}
{/if}
