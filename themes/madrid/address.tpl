...
<div class="box">
	...
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std" id="add_address">
		...
		{foreach from=$ordered_adr_fields item=field_name}
			...
			{if $field_name eq 'phone_mobile'}
				...
				<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
					...
					<input class="validate form-control" data-validate="{$address_validation.phone_mobile.validate}" type="tel" id="phone_mobile" name="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$address->phone_mobile|escape:'html':'UTF-8'}{else}+34{/if}{/if}" />
				</div>
			{/if}
		{/foreach}
		{if !$postCodeExist}
		...
		<p class="submit2">
			...
			<button type="submit" name="submitAddress" id="submitAddress" class="button btn-primary">
				<span>
					{l s='Save'}
				</span>
			</button>
...
<ul class="footer_links clearfix">
	<li>
		<a class="button button-small" href="{$link->getPageLink('addresses', true)|escape:'html':'UTF-8'}">
