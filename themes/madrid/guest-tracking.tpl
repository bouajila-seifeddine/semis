	...
	{else}
		<form method="post" action="{$action|escape:'html':'UTF-8'}#guestToCustomer" class="std">
			<fieldset class="description_box box">
				<h3 id="guestToCustomer" class="page-heading"><span>{l s='For more advantages...'}</span></h3>
				...
				<p>
                    <button type="submit" name="submitTransformGuestToCustomer" class="button btn-primary"><span>{l s='Send'}</span></button>
                </p>
			</fieldset>
		</form>
	{/if}
{else}
	...
	{if isset($show_login_link) && $show_login_link}
		<p><a class="button" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">{l s='Click here to log in to your customer account.'}</a><br /><br /></p>
	{/if}
	<form method="post" action="{$action|escape:'html':'UTF-8'}" class="std" id="guestTracking">
		<fieldset class="description_box box">
			<h2 class="page-heading"><span>{l s='To track your order, please enter the following information:'}</span></h2>
                    <div class="text form-group">
			...
			<p>
                <button type="submit" name="submitGuestTracking" class="button btn-primary"><span>{l s='Send'}</span></button>
            </p>
		</fieldset>
	</form>
{/if}
