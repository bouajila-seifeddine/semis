	...
	{foreach from=$multipleAddresses item=address name=myLoop}
    	<div class="col-xs-12 col-sm-6 address">
			<ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if}{if $smarty.foreach.myLoop.index % 2} alternate_item{else} item{/if} box">
                <li><h3 class="page-heading"><span>{$address.object.alias}</span></h3></li>
                ...
                <a class="button btn-primary button-small" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html':'UTF-8'}" title="{l s='Update'}"><span>{l s='Update'} <i class="icon icon-chevron-right right"></i></span></a>
                <a class="button button-small" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}" onclick="return confirm('{l s='Are you sure?' js=1}');" title="{l s='Delete'}"><span>{l s='Delete'} <i class="icon icon-remove"></i></span></a></li>
            </ul>
        </div>
...
<div class="clearfix main-page-indent">
	<a href="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" title="{l s='Add an address'}" class="button btn-primary"><span>{l s='Add a new address'}</span></a>
</div>
<ul class="footer_links clearfix">
	<li><a class="button button-small" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}"><span><i class="icon icon-chevron-left"></i> {l s='Back to your account'}</span></a></li>
	<li><a class="button button-small" href="{$base_dir}"><span>{l s='Home'}</span></a></li>
</ul>
