					...
					<td class="ajax_block_product comparison_infos product-block product-{$product->id}">
						...
						<div class="product-image-block">
							...
							title="{$product->name|escape:'html':'UTF-8'}">
								...
								src="{$link->getImageLink($product->link_rewrite, $product->id_image, 'medium_default')|escape:'html':'UTF-8'}"
								...
								<div class="button-container">
									{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
										{if ($product->quantity > 0 OR $product->allow_oosp)}
											<a class="button ajax_add_to_cart_button btn-primary" data-id-product="{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" title="{l s='Add to cart'}">
												<span>{l s='Add to cart'}</span>
											</a>
										{else}
											<span class="ajax_add_to_cart_button button disabled">
												<span>{l s='Add to cart'}</span>
											</span>
										{/if}
									{/if}
									<a class="button lnk_view" href="{$product->getLink()|escape:'html':'UTF-8'}" title="{l s='View'}">
										<span>{l s='View'}</span>
									</a>
								</div>
							</div>
...
{/if}
<ul class="footer_link nolist">
	<li>
		<a class="button button-small lnk_view" href="{$base_dir}">
			<span><i class="icon icon-chevron-left left"></i> {l s='Continue Shopping'}</span>
		</a>
	</li>
</ul>
