...
<div class="col-lg-3 col-md-4 hidden-sm hidden-xs shopping_cart  shopping_cart_desktop pull-right{if $PS_CATALOG_MODE} header_user_catalog{/if}">
	<div class="row">
		...
		{else}
		<p>{l s='Welcome visitor! You can' mod='blockcart'} <a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" title="register account">{l s='sign in' mod='blockcart'}</a>.</p>
		{/if}
{/if}
		<a id="showCart" class="cart-contents" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			<span class="pull-left">{l s='Shopping cart' mod='blockcart'}:
				<span class="ajax_cart_quantity{if $cart_qties == 0} unvisible{/if}">{$cart_qties}</span>
				<span class="ajax_cart_product_txt{if $cart_qties != 1} unvisible{/if}">{l s='Product' mod='blockcart'}</span>
				<span class="ajax_cart_product_txt_s{if $cart_qties < 2} unvisible{/if}">{l s='Products' mod='blockcart'}</span>
				...
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
							...
							<dl class="products">
								{foreach from=$products item='product' name='myLoop'}
									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										<a class="cart-images" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}"><img class="img-responsive" src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default')}" alt="{$product.name|escape:'html':'UTF-8'}" /></a>
										<div class="cart-info">
						...
						<p class="clearBoth cart-buttons"> 
							<a id="button_order_cart" class="button" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
		...
		<div class="clearfix">
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				...
				<div class="button-container">
					<span class="continue button" title="{l s='Continue shopping' mod='blockcart'}">
						<span>
							{l s='Continue shopping' mod='blockcart'}
						</span>
					</span>
					<a class="button btn-primary" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
						<span>
