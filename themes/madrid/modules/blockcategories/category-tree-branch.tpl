...
<li {if isset($last) && $last == 'true'}class="last"{/if}>
    ...
    {*{if $node.children|@count > 0}
        <span class='closed' data-href="{$node.link|escape:'html':'UTF-8'}"{if isset($currentCategoryId) && $node.id == $currentCategoryId} class="selected"{/if} title="{$node.name|escape:'html':'UTF-8'}">
            <a class="lc_link" href="{$node.link|escape:'html':'UTF-8'}">{$node.name|escape:'html':'UTF-8'}</a>
        </span>
    {else}*}
