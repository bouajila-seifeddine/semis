		...
		<div class="row simpleblog-posts" itemscope="itemscope" itemtype="http://schema.org/Blog">
			{foreach from=$posts item=post}
				...
				{/if}" itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
					<div class="post-item {if $blogLayout eq 'full'}full{/if}">
						...
						{else}
							<div class="post-additional-info post-meta-info {if $blogLayout eq 'full'}list-margin{/if}">
								{if Configuration::get('PH_BLOG_DISPLAY_DATE')}
									<span class="post-date">
										<time itemprop="datePublished" datetime="{$post.date_add|date_format:'c'}">{$post.date_add|date_format:Configuration::get('PH_BLOG_DATEFORMAT')}</time>
									</span>
								{/if}

								{if $is_category eq false && Configuration::get('PH_BLOG_DISPLAY_CATEGORY')}
									<span class="post-category">
										<a href="{$post.category_url}" title="{$post.category|escape:'html':'UTF-8'}" rel="category">{$post.category|escape:'html':'UTF-8'}</a>
									</span>
								{/if}

								{if isset($post.author) && !empty($post.author) && Configuration::get('PH_BLOG_DISPLAY_AUTHOR')}
									<span class="post-author">
										<span itemprop="author" itemscope="itemscope" itemtype="http://schema.org/Person">{$post.author|escape:'html':'UTF-8'}</span>
									</span>
								{/if}
						...
						{else}
							<div class="post-title {if $blogLayout eq 'full'}list-margin{/if}">
						...
						{else}
							...
							{if $blogLayout eq 'full'}<div class="list-margin">{/if}
								<div class="post-content" itemprop="text">
									{$post.short_content|strip_tags:'UTF-8'}

									{if Configuration::get('PH_BLOG_DISPLAY_MORE')}
									<div class="post-read-more">
										<a class="button btn-primary button-small" href="{$post.url|escape:'html':'UTF-8'}" title="{l s='Read more' mod='ph_simpleblog'}">
											{l s='Read more' mod='ph_simpleblog'} <i class="icon icon-arrow-circle-right"></i>
										</a>
									</div><!-- .post-read-more -->
									{/if}
								</div><!-- .post-content -->	
